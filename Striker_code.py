...
# настройка музыки
pygame.mixer.music.load('AAA.ogg')  # фоновая музыка
pygame.mixer.music.play()

fire_sound = pygame.mixer.Sound('select.ogg')
#шрифты и надписи
pygame.font.init()
...
class Player(GameSprite):
    ...
    def fire(self):
        """метод выстрела"""
        bullet = Bullet('x.jpg', self.rect.centerx,self.rect.top, -10, 25, 25)
        bullets.add(bullet)
...
# создаем спрайты
ship = Player('rog.jpg', WIDTH/2.2, HEIGHT - 80, 4, 80, 80)  # картинка, расположение по x, расположение по y, скорость

monsters = pygame.sprite.Group()
for i in range(1, 4):
    monster = Enemy('svinki.jpeg', random.randint(80, WIDTH - 80), -40, 2, 65, 65)
...
for i in range(1, 4):
    asteroid = Enemy('stone.png', random.randint(80, WIDTH - 80), -40, 2, 65, 65)
...
while True:
    ...
    if not finish:
        ...
        for c in collides:
            score = score + 1
            monster = Enemy('svinki.jpeg', random.randint(80,WIDTH - 80), -40, 3, 65, 65)
            monsters.add(monster)
